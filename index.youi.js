...
//this variable contains the original error handler
const defaultErrorHandler = ErrorUtils.getGlobalHandler();

const myErrorHandler = (e, isFatal) => {
  //console.error('constructor',e.constructor); //Function that invoque the error
  //console.error('error',e); // Complete Error object
  console.error("line", e.line); // Line when error has ocurred
  console.error("column", e.column); // Column when error has ocurred
  console.error("sourceURL", e.sourceURL); // File when error has ocurred
  console.error("message", e.message); // Error message
  console.error("name", e.name); // Error message
  console.error("stack", e.stack.toString()); // Stack trace from error
  console.error("is fatal ?", isFatal); // if true, the error will close the app

  //send error to NativeModules
  NewRelicBridge.crashJS(
    e.name,
    e.message,
    e.line,
    e.column,
  ...
  //enable this function to see the original developer error handler
...
export default class YiReactApp extends Component {
  constructor(props) {
    ...
    NewRelicBridge.recordCustomEvent("LifeCycleEvent", {
  ...
  componentDidMount() {
    NewRelicBridge.recordCustomEvent("LifeCycleEvent", {
      eventName: "componentDidMount",
    });

    fetch("https://en.wikipedia.org/wiki/New_Relic", {})
      .then((response) => {
        NewRelicBridge.recordCustomEvent("NetworkRequest", {
          eventName: "fetch",
          url: response.url,
      ...
      .catch((error) => {
        NewRelicBridge.recordCustomEvent("NetworkError", {
  ...
  crashAsync() {
    NewRelicBridge.crashAsync("force crash async");
  }

  componentWillMount() {
    NewRelicBridge.recordCustomEvent("LifeCycleEvent", {
      eventName: "componentWillMount",
    });
  }
